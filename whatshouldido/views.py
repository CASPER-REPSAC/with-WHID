...
def check_auth_user_id_exist(request):
    if '_auth_user_id' in dict(request.session):
        return True
    log.error("Error Occurs : views.check_auth_user_id_exist False | User :"+ request.user.id )
    return False
...
def getUserContents(user_id):
    ...
    log.info("getUserContents | User :"+ user_id)
    return {
            "data": {
    ...
    #try:
        ...
        if check_auth_user_id_exist(request):
            if request.method == 'GET':
                ...
                log.info("User Login | User :"+ user_id)
...
def calendarDetail(request, date_time: str):
    try:
        ...
        log.error("Error Occurs calendarDetail.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
        ...
        if check_auth_user_id_exist(request):
            ...
            if request.method == "POST":
                ...
                if user_id.pk != group.groupmaster.pk and category == 1:
                    log.warning("groupArticleCreate| User: "+user_id.pk + " is not group master." )
                    return redirect('whatshouldido:error')
                ...
                if article_form.is_valid():
                    ...
                    article.save()
                    log.info("Article has written | User: "+user_id+" Group : "+group_id)
            ...
            context.update(getUserContents(user_id))
            log.info("groupArticleCreate | User, Group :"+ user_id+", "+group_id)
            return render(request, "group-article-write.html", context)
        else:
            return redirect('whatshouldido:index')
    except:
        log.error("Error Occurs groupArticleCreate.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupArticleList(request, group_id):
    try:
        ...
        log.error("Error Occurs groupArticleList.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupArticleRead(request, group_id, article_id):
    try:
        ...
        log.error("Error Occurs groupArticleRead.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
        ...
        if check_auth_user_id_exist(request):
            ...
            if request.method == "POST":
                ...
                if comment_form.is_valid():
                    ...
                    comment.save()
                    log.info("commentCreate | User, Article :"+ user_id+", "+article_id)
                    return redirect('whatshouldido:group-article-read', group_id=group_id, article_id=article_id)
            return redirect('whatshouldido:group-article-read', group_id=group_id, article_id=article_id)
        else:
            return redirect('whatshouldido:index')
    except:
        log.error("Error Occurs commentCreate.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
        ...
        if check_auth_user_id_exist(request):
            ...
            if request.method == "POST":
                ...
                if user_id == comment.writer:
                    comment.delete()
                    log.info("commentDelete | User, Article :"+ user_id+", "+article_id)
                    return redirect('whatshouldido:group-article-read', group_id=group_id, article_id=article_id)
            return redirect('whatshouldido:group-article-read', group_id=group_id, article_id=article_id)
        else:
            return redirect('whatshouldido:index')
    except:
        log.error("Error Occurs commentDelete.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupArticleEdit(request, group_id, article_id):
    try:
        if check_auth_user_id_exist(request):
            ...
            if request.method == "POST":
                ...
                if article_form.is_valid():
                    ...
                    article.groupid = get_object_or_404(Studygroups, pk=group_id)
                    article.save()
                    log.info("groupArticleEdit | User, Group :"+ user_id+", "+group_id)
                    return redirect('whatshouldido:group-article-read', group_id=group_id, article_id=article.pk)
        ...
        log.error("Error Occurs groupArticleEdit.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
    ...
    try:
        if check_auth_user_id_exist(request):
            ...
            if user_id == article.userid:
                article.delete()
                log.info("groupArticleDelete | User, Group :"+ user_id+", "+group_id)
                return redirect('whatshouldido:group-article-list', group_id=group_id)
        ...
        log.error("Error Occurs groupArticleDelete.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupAssignmentList(request, group_id):
    try:
        ...
        log.error("Error Occurs groupAssignmentList.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupAssignmentCreate(request, group_id):
    try:
        if check_auth_user_id_exist(request):
            ...
            context.update(getUserContents(user_id))
            log.info("groupAssignmentCreate | User, Group :"+ user_id+", "+group_id)
            return render(request, "group-assign-write.html", context)
        else:
            return redirect('whatshouldido:index')
    except:
        log.error("Error Occurs groupAssignmentCreate.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupAssignmentEdit(request, group_id, assign_id):
    try:
        if check_auth_user_id_exist(request):
            ...
            context.update(getUserContents(user_id))
            log.info("groupAssignmentEdit | User, Assignment :"+ user_id+", "+assign_id)
            return render(request, "group-assign-write.html", context)
        else:
            return redirect('whatshouldido:index')
    except:
        log.error("Error Occurs groupAssignmentEdit.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupAssignmentRead(request, group_id, assign_id):
    try:
        ...
        log.error("Error Occurs groupAssignmentRead.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
    ...
    try:
        if check_auth_user_id_exist(request):
            ...
            if user_id.pk == group.groupmaster.id:
                ...
                log.info("groupAssignmentDelete | User, Assignment :"+ user_id+", "+assign_id)
                return redirect('whatshouldido:group-assign-list', group_id=group_id)
        else:
            return redirect('whatshouldido:index')
    except:
        log.error("Error Occurs groupAssignmentDelete.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupSearch(request):
    try:
        ...
        log.error("Error Occurs groupSearch.except 1| User: "+request.user.id)
        return redirect('whatshouldido:error')
...
def groupCreate(request):
    try:
        ...
        log.error("Error Occurs groupCreate.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def groupManage(request, group_id):
    try:
        ...
        log.error("Error Occurs groupManage.except 1| User: "+user_id)
        return redirect('whatshouldido:error')
...
def uploadFile(request):
    if request.method == "POST":
        ...
        file.save()
    log.info("file uploaded | File Num :" +file.field_encr_filename)
